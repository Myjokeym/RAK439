/**
  ******************************************************************************
  * @file    rw_lib_platform.c
  * @author  RAK439 module Design Team
  * @version V1.0.2
  * @date    13-Jun-2015
  * @brief   RAK439 module non-OS Demo rw_lib_platform Application C File.
  *
  *          This file contains:
...
static void ip_callback(rw_IpConfig_t* ip_addr, int status)
{
    ...
    {   
       DPRINTF("ipquery fail code =%d\r\n", status);
       app_demo_ctx.rw_ipquery_status =STATUS_FAIL; 
    }
}
static void dns_ipcallback(int dns_ip)
{
    if(dns_ip !=0)
    {
       DPRINTF("dns success addr = 0x%x\r\n", dns_ip);
    }else
    {   
       DPRINTF("dns fail ,please retry!!!\r\n");
    }  
}
...
void wifi_init_params(rw_DriverParams_t* params)
{
    ...
    params->app_cb.dhcp_cb = ip_callback;
    params->app_cb.dns_cb = dns_ipcallback;
...
static uint32_t _init_interface(void)
{
    WIFI_GPIO_Init();
    WIFI_SPI_Init();
    WIFI_INT_Init();  //need init if params->spi_int_enable is true .
